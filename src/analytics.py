...
import pandas as pd
from .config import PRESET_QUERIES, PRESET_PAGE_SIZE, STATUS_FOCUS, TARGET_LANES, TODAY, LANE_SCAN_OPTIONS, LANE_QUERY_MAP
from .clinicaltrials_client import fetch_registry_studies


def _normalize_scan_lanes(scan_scope="All 4 lanes") -> list[str]:
    """Return selected lane names from either legacy single-scope labels or the new multi-lane picker."""
    if isinstance(scan_scope, (list, tuple, set)):
        lanes = [lane for lane in scan_scope if lane in TARGET_LANES]
        return lanes or list(TARGET_LANES.keys())
    scope = LANE_SCAN_OPTIONS.get(scan_scope, LANE_SCAN_OPTIONS["All 4 lanes"])
    return [lane for lane in scope.get("lanes", list(TARGET_LANES.keys())) if lane in TARGET_LANES]


def _scan_label(lane_names: list[str]) -> str:
    if len(lane_names) == len(TARGET_LANES):
        return "All 4 lanes"
    return " + ".join(lane_names)


def run_clinical_analysis(scan_scope="All 4 lanes") -> dict:
    lane_names = _normalize_scan_lanes(scan_scope)
    queries = [LANE_QUERY_MAP[lane] for lane in lane_names if lane in LANE_QUERY_MAP] or PRESET_QUERIES
    scan_label = _scan_label(lane_names)

    df = fetch_registry_studies(queries, PRESET_PAGE_SIZE)
    if len(lane_names) == len(TARGET_LANES):
        df = df[df["target_lane"].isin(lane_names) | (df["target_lane"] == "Other / Unclassified")].copy()
    else:
        # Custom lane mixes should stay crisp: suppress broad unclassified spillover so
        # every downstream chart/table remains specific to the selected lanes.
        df = df[df["target_lane"].isin(lane_names)].copy()
    ...
    return {
        ...
        "scan_scope": scan_label,
        "scan_lanes": lane_names,
        "scan_caption": scan_label,
    }
