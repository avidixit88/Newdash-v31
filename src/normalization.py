...
import pandas as pd
from .config import ACTIVE_STATUSES, PLANNED_STATUSES, TARGET_LANES, COMBO_CLASSIFIERS, COMBO_STRUCTURE_TERMS, TODAY, LANE_STRATEGY_RULES, NON_ONCOLOGY_CLASSIFIERS, ADC_TERMS, ADC_ASSET_TERMS, NON_ADC_MODALITY_TERMS, ONCOLOGY_ADC_LANES, STATUS_GROUP_MAP, STRATEGIC_SIGNAL_PATTERNS
...
def classify_status_group(status: str | None) -> str:
    ...
    Defensive note: a prior build accidentally returned a tuple containing the
    whole STATUS_GROUP_MAP dictionary. Pandas cannot group/hash dict-bearing
    tuples, which caused Streamlit Cloud TypeErrors in status charts. This
    function must always return a plain scalar string.
    """
...
def extract_strategic_signal_stack(row: pd.Series) -> str:
    """Target-specific biology + regimen-design signals for executive charts.

    This intentionally keeps B7-H4/CDH6 visible even when the registry record
    does not list a named external partner agent. A B7-H4 ADC monotherapy study
    is still important to Michael, but it should not be hidden just because it
    lacks pembrolizumab/chemo/etc.
    """
    text = _lot_context_text(row)
    lane = safe_str(row.get("target_lane", ""))
    hits = _match_labels(text, STRATEGIC_SIGNAL_PATTERNS)

    # Lane-aware fallback so target lanes do not disappear from partner/biology
    # graphs when no named partner agent is present.
    if lane == "B7-H4 / VTCN1" and "B7-H4 target expression" not in hits:
        hits.insert(0, "B7-H4 lane captured")
    if lane == "CDH6" and "CDH6 target expression" not in hits:
        hits.insert(0, "CDH6 lane captured")
    if lane == "Alzheimer's / ApoE4" and not any("ApoE4" in h or "Alzheimer" in h for h in hits):
        hits.append("ApoE4 / Alzheimer lane captured")
    if lane == "Bone / Siglec-15" and not any("Bone" in h or "Siglec" in h for h in hits):
        hits.append("Bone / Siglec-15 lane captured")

    return ", ".join(dict.fromkeys(hits)) if hits else "No strategic signal detected beyond registry capture"


def extract_partner_landscape_bucket(row: pd.Series) -> str:
    """Executive-friendly partner bucket that preserves no-partner records."""
    classes = [c.strip() for c in safe_str(row.get("combo_classes", "")).split(",") if c.strip()]
    ...
    modality = safe_str(row.get("modality_class", ""))
    lane = safe_str(row.get("target_lane", ""))
    if lane in ONCOLOGY_ADC_LANES and modality == "ADC / antibody-drug conjugate":
        return "ADC monotherapy / no partner listed"
    if lane in ONCOLOGY_ADC_LANES:
        return "No named partner / modality unclear"
    return "Non-oncology lane signal"
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["strategic_signal_stack"] = df.apply(extract_strategic_signal_stack, axis=1)
    df["partner_landscape_bucket"] = df.apply(extract_partner_landscape_bucket, axis=1)
